...
def _build_snapshot(profile_name: str, rows: list[tuple]) -> SchemaSnapshot:
    ...
    Rows are grouped by the `(TABLE_SCHEMA, TABLE_NAME)` tuple (never a
    concatenated string) to preserve qualified table identity. Ordering is
    applied here, in Python, independent of the row delivery order from
    the catalog query.
    """
