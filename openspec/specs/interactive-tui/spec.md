...
- Q: Does editing the exclude list in the TUI persist back to
  ...
  → A: **No.** The exclude list edited in the TUI exists only in memory
  for the current session/run. No file is read for or written by this
  editing action (REQ-interactive-tui-011).
- Q: Does committing an exclude-list edit automatically re-run the
  comparison, or does the user need a separate action?
  → A: Committing an edit (`Enter`) both updates the in-memory exclude
  list and immediately triggers a comparison re-run using it
  (REQ-interactive-tui-011, REQ-interactive-tui-012). A separate binding
  (`r`) re-runs the comparison without changing the exclude list, for
  observing live schema drift since launch.
- Q: When `--tui` launches, does the pre-existing unconditional
  HTML/PDF/Excel generation at startup still happen, in addition to the
  new on-demand "generate reports" action?
  → A: **No.** Per `reporting-and-output` REQ-reporting-and-output-002,
  automatic upfront generation is skipped specifically when the
  interactive TUI launches; those formats are produced only via the new
  in-TUI "generate reports" action (REQ-interactive-tui-013). Every other
  invocation shape is unaffected.
