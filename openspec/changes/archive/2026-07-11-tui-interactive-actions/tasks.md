...
- [x] 2.3 (GREEN) Implement `_resolve_summary_renderer_and_generate_reports`
      ...
      `main()`'s call site
- [x] 2.4 (RED) Add `test_main_deduplicates_extract_filter_compare_via_run_comparison`
      ...
      directly) — completed as part of Phase 3 task 3.6, once
      `tui/actions.py` existed
- [x] 2.5 (GREEN) Update `main()` to call `run_comparison(profiles, exclude_patterns)`
      ...
      this task) — completed as part of Phase 3 task 3.6
...
- [x] 3.1 (RED) Create `tests/unit/tui/test_actions.py`; write
...
- [x] 3.2 (GREEN) Implement `run_comparison(profiles, exclude_patterns)`
      in `src/schema_comparator/tui/actions.py` per design §2
- [x] 3.3 (RED) Write `test_run_comparison_propagates_extraction_error_unmodified`
      (mock `extract_schema` to raise, assert the same exception type/
      message propagates, no wrapping)
- [x] 3.4 (GREEN) Confirm `run_comparison` has no `try/except` (already
      satisfied by 3.2's implementation; this task is a verification step)
- [x] 3.5 Run `pytest tests/unit/tui/test_actions.py` and confirm all pass
- [x] 3.6 Complete the `cli.py`/`main()` de-duplication deferred from task
...
- [x] 4.1 (RED) Add `tests/unit/tui/test_widgets.py` tests (or extend it
...
- [x] 4.2 (GREEN) Implement `StatusLog` and `ExcludeEditor` in
      `src/schema_comparator/tui/widgets.py` per design §3
- [x] 4.3 Run `pytest tests/unit/tui/test_widgets.py` and confirm all pass

## Phase 5: `tui/app.py` — Bindings, Workers, Wiring

- [x] 5.1 (RED) Add `test_app_accepts_profiles_and_exclude_patterns_constructor_args`
      to `tests/unit/tui/test_app.py` (REQ-interactive-tui-011)
- [x] 5.2 (GREEN) Update `SchemaComparatorApp.__init__`/`compose` per
      design §4 (new keyword-only `profiles`/`exclude_patterns` params;
      mount `ExcludeEditor` and `StatusLog`)
- [x] 5.3 (RED) Write `test_pressing_e_focuses_exclude_editor` (`Pilot`,
      `@pytest.mark.asyncio`) (REQ-interactive-tui-011)
- [x] 5.4 (GREEN) Implement `action_focus_exclude_editor`
- [x] 5.5 (RED) Write `test_submitting_exclude_editor_updates_patterns_and_triggers_run`
...
- [x] 5.6 (GREEN) Implement `on_input_submitted` per design §4
- [x] 5.7 (RED) Write `test_pressing_r_triggers_run_comparison_without_changing_excludes`
      (REQ-interactive-tui-012)
- [x] 5.8 (GREEN) Implement `action_run_comparison`/`_do_run_comparison`/
      `_apply_new_result` per design §4, using `run_worker(..., thread=True)`
      and `call_from_thread` for all UI updates
- [x] 5.9 (RED) Write `test_run_comparison_failure_leaves_previous_result_and_logs_error`
...
- [x] 5.10 (GREEN) Confirm/adjust `_do_run_comparison`'s `except` branch
      returns before calling `_apply_new_result` (already satisfied by
      5.8's implementation; verification task)
- [x] 5.11 (RED) Write `test_pressing_g_calls_generate_reports_with_string_io_and_logs_output`
...
- [x] 5.12 (GREEN) Implement `action_generate_reports`/`_do_generate_reports`
      per design §4
- [x] 5.13 (RED) Write `test_generate_reports_single_format_failure_does_not_crash_app`
      ...
      scenario)
- [x] 5.14 (GREEN) Confirm no unhandled exception path exists between
      `_do_generate_reports` and the worker (verification task; adjust if
      the fake surfaces a gap)
- [x] 5.15 Run `pytest tests/unit/tui/test_app.py` in full and confirm all
      Phase 5 tests pass together with the pre-existing (archived-change)
      tests in that file

## Phase 6: `run_tui` Passthrough + Final Wiring

- [x] 6.1 (RED) Add `test_run_tui_forwards_profiles_and_exclude_patterns`
...
- [x] 6.2 (GREEN) Update `run_tui`'s signature in `tui/app.py` (and its
...
- [x] 6.3 Run the full suite with `pytest --cov`; confirm `asyncio_mode =
